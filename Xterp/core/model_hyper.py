...
class LSTMHyperModel(HyperModel):
	...
	def build(self, hp):
		...
		for layer in configs['model']['layers']:
			...
			if layer['type'] == 'dense':
				model.add(Dense(neurons, activation=activation)) #leave this one alone for now
			if layer['type'] == 'lstm':
		...
		model.compile(loss=configs['model']['loss'], optimizer=hp.Choice(
			'optimizer',
			values=['sgd', 'adam'],
			default=configs['model']['optimizer']
	...
	def train(self, model, x, y, epochs, batch_size, save_dir):
		timer = Timer()
		timer.start()
	...
	def train_generator(self, model, data_gen, epochs, batch_size, steps_per_epoch, save_dir):
		timer = Timer()
		timer.start()
		print('[Model] Training Started')
