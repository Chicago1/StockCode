...
class DataLoader():
    ...
    def __init__(self, filename, train_split, valid_split):
    
        dataframe = pd.read_csv(filename) 

        num_train_samples = int(len(dataframe) * (train_split))
        
        num_valid_samples = int(len(dataframe) * (valid_split))

        self.data_train = dataframe.to_numpy()[:num_train_samples,1:] #creates train dataset from file (beginning to i_split)

        self.data_valid = dataframe.to_numpy()[num_train_samples:num_train_samples+num_valid_samples,1:]

        self.data_test  = dataframe.to_numpy()[num_train_samples+num_valid_samples:,1:]
        ...
        assert(self.len_train + self.len_valid + self.len_test == len(dataframe.to_numpy()[:,1:]))
    
        self.len_train_windows = None #TODO
